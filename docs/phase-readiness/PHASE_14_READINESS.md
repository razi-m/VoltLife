...
## Phase Goal
- Provide simulated SaaS subscription billing for suppliers (Monthly, Annual, Enterprise plans) via a mock or test-mode Razorpay checkout.
...
- Endpoints to add under `/api/v1/subscriptions`:
  ...
  - `POST /api/v1/subscriptions/create-session`
    ...
    - Output: `{ "session_id": str, "checkout_url": str }` (simulating Razorpay checkout redirect or returning mock checkout params).
  - `POST /api/v1/subscriptions/verify`
    - Input: `{"session_id": str}` (or `{"razorpay_payment_id": str, "razorpay_signature": str}` for real-test integration).
...
## Team Ownership
- **Backend Team**: Implement subscription endpoints, mock Razorpay payment adapters, database persistence, and integration of subscription gating checks.
...
- Razorpay Subscriptions (Mockable / TEST mode only via public keys).
...
- **Mock vs. Real Test mode transition**:
  - *Mitigation*: Provide a fallback mock mode when `RAZORPAY_KEY_ID` or `RAZORPAY_KEY_SECRET` are not set. The mock flow must operate cleanly and automatically update the database state to `"active"`.
...
- Suppliers can purchase a subscription using simulated Razorpay flow (test mode checkout or mock checkout).
