...
export const Sidebar: React.FC<SidebarProps> = ({ wsConnected }) => {
  ...
  const navItems = isSupplier ? sellerNavItems : buyerNavItems;
  ...
  return (
    <aside className="sidebar">
      ...
      <nav className="sidebar__nav">
        <div className="sidebar__section-label text-label-caps">
          {isSupplier ? 'Operations' : 'Browse'}
        </div>
        {navItems.map((item) => (
          <NavLink
            key={item.path}
